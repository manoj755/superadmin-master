// JavaScript Document
app.controller('emailunsubscriberCtrl', function ($scope, $rootScope, db, $mdDialog) {
    ...
    $scope.Edit = function (row) {
        ...
        $scope.isEdit = false;
           $scope.isEdit = true;   $scope.message=row;
        db.show("emailunsubscriber/", row.id, function (response) {
            debugger;
            $scope.isEdit = true;
...
//            $scope.gridTotalJobs.data = response.data;
    ...
    $scope.Delete = function (row) {
        ...
        if (confirm('Are you sure?')) {
            db.destroy("emailunsubscriber/", row.id, function (response) {
...
//                cellTemplate: '<a class="btn primary"  href="javascript:" data-toggle="modal" data-target="#myModal6" ng-click="grid.appScope.ShowTotalCandidatejob(row.entity)">{{row.entity.Total_Candidate}}</a>'}
    ...
    $scope.gridOptions = {
        columnDefs: [
            ...
            {name: 'email'},{name: 'created_at'},{name: 'updated_at'},{name: 'ipAddress'},
    ...
    $scope.getlist = function () {
        ...
        db.list("emailunsubscriber/", null, function (response) {
    ...
    $scope.messagesave = function () {
        if ($('.validate').validate('#messagetemp', true)) {
            ...
            db.store("emailunsubscriber/", $scope.message, function (response) {
    ...
    $scope.showuser = function (id) {
        //$scope.job = job; 
        db.show("emailunsubscriber/", id, function (response) {
    ...
    $scope.messageupdate = function () {
        db.update("emailunsubscriber/", $scope.message.id, $scope.message, function (response) {
